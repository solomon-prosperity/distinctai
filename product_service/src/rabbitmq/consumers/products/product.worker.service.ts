...
@Injectable()
export class ProductWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly queuePrefix = `${process.env.RABBITMQ_QUEUE_PREFIX}.product.service`;
  ...
  private async handleMessage(channel: ConfirmChannel, msg: ConsumeMessage) {
    ...
    try {
      ...
      switch (eventName) {
        case 'update_product_owner_details':
          this.logger.info(' [Received] %s', eventName);
          await this.productService.updateProduct(message.data);
